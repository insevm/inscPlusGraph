...
export class Inscribe extends Entity {
  constructor(id: Bytes) {
    super();
    this.set("id", Value.fromBytes(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save Inscribe entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type Inscribe must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`,
      );
      store.set("Inscribe", id.toBytes().toHexString(), this);
    }
  }

  static loadInBlock(id: Bytes): Inscribe | null {
    return changetype<Inscribe | null>(
      store.get_in_block("Inscribe", id.toHexString()),
    );
  }

  static load(id: Bytes): Inscribe | null {
    return changetype<Inscribe | null>(store.get("Inscribe", id.toHexString()));
  }
  ...
  get INSCPlus_id(): BigInt {
    let value = this.get("INSCPlus_id");
    if (!value || value.kind == ValueKind.NULL) {
      throw new Error("Cannot return null for a required field.");
    } else {
      return value.toBigInt();
  ...
  set INSCPlus_id(value: BigInt) {
    this.set("INSCPlus_id", Value.fromBigInt(value));
  }
